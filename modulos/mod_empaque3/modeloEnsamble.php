...
<?
		...
		public function verificarPO($idEntregaInterna,$conceptoEntrega,$modelo,$usrEmpaque,$idElemento,$valores,$numeroCaja,$cantidadCapturada,$idModeloCaptura,$poAValidar,$idValidacionAgrupada){
			$array3619=array("01176","01177","01178","01184","01185","01193","01195","01198","01199","01204","01207","01208","01212","01213","01218","01219","01226","01230","01235","01236","01239","01242","01245","01249","01255","01258","01260","01263","01267","01274","01275","01276","01284","01290","01291","01296","01300","01303","01309","01311","01312","01319","01324","01331","01344","01345","01353","01354","01355");
			$array2390=array("01356","01360","01364","01365","01375","01379","01385","01389","01394","01398","01401","01403","01408","01409","01414","01420","01421","01427","01433","01439","01445","01451","01456","01465");						
			$sqlId="SELECT id_radio,status,lote FROM equipos WHERE imei='".$valores."' AND id_modelo='".$idModeloCaptura."' AND status in('Validando','WIP') AND statusProceso in ('Empaque','Empacado','Validacion')";
			...
			if(mysql_num_rows($resId) != 0){
				...
				//folio del equipo
				//echo "<br>".$rowId["lote"];
				if(substr($poAValidar,5)=="3619"){					
					if(in_array($rowId["lote"],$array3619)){
						/*la informacion es correcta*/
						echo "<br>el equipo pertenece a la PO 2390";
						$this->guardaEquiposEmpaqueFinal($idEntregaInterna,$conceptoEntrega,$modelo,$usrEmpaque,$idElemento,$valores,$numeroCaja,$cantidadCapturada,$idModeloCaptura,$poAValidar,$idValidacionAgrupada);
					}else{
						echo "<br><span style='color:red;font-weight:bold;font-size:14px;'>Este Equipo no se puede capturar con el numero de OCA actual</span>";
					}
				}else if(substr($poAValidar,5)=="2390"){
					if(in_array($rowId["lote"],$array2390)){
						/*la informacion es correcta*/
						echo "<br>el equipo pertenece a la PO 2390";
						$this->guardaEquiposEmpaqueFinal($idEntregaInterna,$conceptoEntrega,$modelo,$usrEmpaque,$idElemento,$valores,$numeroCaja,$cantidadCapturada,$idModeloCaptura,$poAValidar,$idValidacionAgrupada);
					}else{
					...
					$validacionFolioActual=false; $msgValidacion="";
					//se busca el folio en los anteriores arrays
					if(in_array($rowId["lote"],$array3619)){
						$msgValidacion="<span style='color:red;font-weight:bold;font-size:14px;'>Este equipo se debe capturar con la PO 120113619</span>";
						$validacionFolioActual=true;
					}
					if(in_array($rowId["lote"],$array2390)){
						$msgValidacion="<span style='color:red;font-weight:bold;font-size:14px;'>Este equipo se debe capturar con la PO 120122390</span>";
						$validacionFolioActual=true;
					}
					if($validacionFolioActual){
						echo "<br>".$msgValidacion;
					}else{
						echo "<br>el equipo se puede guardar";
						$this->guardaEquiposEmpaqueFinal($idEntregaInterna,$conceptoEntrega,$modelo,$usrEmpaque,$idElemento,$valores,$numeroCaja,$cantidadCapturada,$idModeloCaptura,$poAValidar,$idValidacionAgrupada);
					}
				}
			}else{
				echo "<br>Verifique la informaci&oacute;n del equipo.<br>";
			}
		}
		
		public function guardaEquiposEmpaqueFinal($idEntregaInterna,$conceptoEntrega,$modelo,$usrEmpaque,$idElemento,$valores,$numeroCaja,$cantidadCapturada,$idModeloCaptura,$poAValidar,$idValidacionAgrupada){
			//se busca que el equipo pertenezca a la validacion que se subio anteriormente
			$sqlValAgrupada="SELECT  imei FROM empaque_validaciones_archivo WHERE id_empaque_validaciones='".$idValidacionAgrupada."' AND imei='".$valores."'";
			$resValAgrupada=mysql_query($sqlValAgrupada,$this->conectarBd());
			if(mysql_num_rows($resValAgrupada)!=0){				
				//se valida que su status sea ok dependiendo del archivo de validacion			
				$sqlValidacion="SELECT * FROM empaque_items WHERE imei='".$valores."'";
				$resValidacion=mysql_query($sqlValidacion,$this->conectarBd());
				$rowValidacion=mysql_fetch_array($resValidacion);
				if($rowValidacion["statusEntrega"]=="NOK"){
					echo "Este equipo no se puede entregar";
					$msgCaja="VERIFIQUE VALIDACION";
					$color="red";
					$fuente="white";
				...
				}else if($rowValidacion["statusEntrega"]=="OK"){
				...
				echo "<br><span style='color:red;font-weight:bold;font-size:14px;'>Este Equipo NO PERTENECE a esta VALIDACION</span>";				
				$msgCaja="VERIFIQUE VALIDACION";
				$color="red";
				$fuente="white";
				echo "<script type='text/javascript'>document.getElementById('".$idElemento."').value='".$msgCaja."'; </script>";
		...
		public function eliminarEntrega($idEntregaInterna,$cantidadEntrega,$txtIdUsuarioEmpaque,$idValidacion){			
			...
			if($resElimina){
				...
				//se elimina las entregas relacionadas
?>
...
<?
		...
		public function formularioEntregas($idModelo,$idEntregaInterna,$cantidadEquiposEmpacados,$idValidacion){
			...
			$resModelo=mysql_query($sqlModelo,$this->conectarBd());
			$rowModelo=mysql_fetch_array($resModelo);
			$sqlOcas="SELECT valor FROM configuracionglobal WHERE nombreConf='ocas_nextel'";
			$resOcas=mysql_query($sqlOcas,$this->conectarBd());
			$rowOcas=mysql_fetch_array($resOcas);
			$ocas=explode("|",$rowOcas["valor"]);
?>
			...
			<table width="375" border="0" cellpadding="1" cellspacing="1" style="margin: 5px;">
				...
				<tr>
					<td style="height:25px; text-align:left; border-bottom:1px solid #CCC;font-weight:bold; border:1px solid #CCC; background:#F0F0F0;">P.O.:</td>
					<td><!--<input type="text" name="poEntrega" id="poEntrega">-->
					<select name="poEntrega" id="poEntrega">
						<option value="">Selecciona...</option>
<?
			for($i=0;$i<count($ocas);$i++){
?>
					<option value="<?=$ocas[$i];?>"><?=$ocas[$i];?></option>
<?
			}
?>
					</select>
...
?>
					<div id="barraOpcionesEmpaque" style="width:99%;border:1px solid #CCC;background:#e1e1e1;height:20px;padding:5px;">
						...
						<a href="#" onclick="verificaEquiposMarcados('<?=$id?>')" title="Verificar Marcacion de Equipos Enviados" style="color:blue; font-size:10px;text-decoration: none;">Validar Marcacion de Equipos Enviados</a>
					</div>
...
?>
						<tr>
							<td style="background:<?=$color1;?>;height:15px;padding:5px;text-align:center;border-bottom:1px solid #666;border-top:1px solid #666;font-weight:bold;"><?=$rowResumenPrevio["modeloRadio"];?></td>
							<td style="background:<?=$color1;?>;height:15px;padding:5px;text-align:center;border-bottom:1px solid #666;border-top:1px solid #666;"><?=$rowResumenPrevio["Filas"];?></td>
							<td style="background:<?=$color1;?>;height:15px;padding:5px;text-align:center;border-bottom:1px solid #666;border-top:1px solid #666;"><?=$entregablesOk1;?></td>
							<td style="background:<?=$color1;?>;height:15px;padding:5px;text-align:center;border-bottom:1px solid #666;border-top:1px solid #666;"><?=$entregablesNok1;?></td>
							<td style="background:<?=$color1;?>;height:15px;padding:5px;text-align:center;border-bottom:1px solid #666;border-top:1px solid #666;"><?=$equiposEmpacados1;?></td>
							<td style="background:<?=$color1;?>;height:15px;padding:5px;text-align:center;border-bottom:1px solid #666;border-top:1px solid #666;">
<?
...
?>
						...
						<tr>
							<td colspan="6" style="background: #bdbdbd;">							
<?
						if(mysql_num_rows($resEntregasporEntrega)==0){
							echo "<span style='font-size:14px;color:red;font-weight:bold;'>No se han capturado entregas.</span>";
						}else{
?>
...
?>
								<tr class="resultadosEntregas" style="background: #A9BCF5;">
									...
									<a href="#" onclick="detalleEntregaListado('<?=$rowEntregaporEntrega["id"];?>','<?=$rowEntregaporEntrega["concepto"];?>')" title="Ver Detalle de la Entrega" style="color:blue;"><img src="../../img/tablaResultados.png" border="0"></a> |
...
?>
										<table border="0" cellpading="1" cellspacing="1" width="96%" style="margin-left: 30px;">
											<tr>
												<td width="10%" style="height:25px; text-align:center; border-bottom:1px solid #CCC;font-weight:bold; border:1px solid #CCC; background:#e6e6e6;">Caja</td>
												<td width="10%" style="height:25px; text-align:center; border-bottom:1px solid #CCC;font-weight:bold; border:1px solid #CCC; background:#e6e6e6;">Cantidad</td>
												<td width="10%" style="height:25px; text-align:center; border-bottom:1px solid #CCC;font-weight:bold; border:1px solid #CCC; background:#e6e6e6;">Limite</td>
												<td width="30%" style="height:25px; text-align:center; border-bottom:1px solid #CCC;font-weight:bold; border:1px solid #CCC; background:#e6e6e6;">Opciones</td>												
											</tr>
<?
...
?>
											<tr class="resultadosEntregasCajas">
												...
												<td style="text-align: center;height:15px;padding:5px;text-align:center;border-bottom:1px solid #666;border-left:1px solid #666;"><?=$rowCapturadosEntrega["total"];?></td>
												<td style="text-align: center;height:15px;padding:5px;text-align:center;border-bottom:1px solid #666;border-left:1px solid #666;"><?=$rowLimitesCajas["limiteDeCapturaCaja"];?></td>
												<td style="text-align: center;height:15px;padding:5px;text-align:center;border-bottom:1px solid #666;border-left:1px solid #666;">
<?
...
?>
											<a href="#" onclick="capturarEntrega('<?=$rowEntregaporEntrega["id"];?>','<?=$rowEntregaporEntrega["concepto"]?>','<?=$rowResumenPrevio["modeloRadio"]?>','<?=$id?>','<?=$rowCajasCaptura["numeroCaja"];?>','<?=$rowCapturadosEntrega["total"];?>','<?=$rowResumenPrevio["idModelo"];?>','<?=$rowEntregaporEntrega["po"];?>','<?=$id;?>')" style="color:blue;">Capturar Caja</a>
<?
											}else if($rowCapturadosEntrega["total"] !=0){//se continua con la captura previamente realizada para una nueva
?>
											<a href="#" onclick="capturarEntrega('<?=$rowEntregaporEntrega["id"];?>','<?=$rowEntregaporEntrega["concepto"]?>','<?=$rowResumenPrevio["modeloRadio"]?>','<?=$id?>','<?=$rowCajasCaptura["numeroCaja"];?>','<?=$rowCapturadosEntrega["total"];?>','<?=$rowResumenPrevio["idModelo"];?>','<?=$rowEntregaporEntrega["po"];?>','<?=$id;?>')" style="color:blue;">Usar Caja Actual</a>
<?
											}
										}else{
