<?php
require_once(dirname(__FILE__) . "/Herramientasconn.php");
...
function findAll() {
	 ...
	 */
	$fields = array('id','codigo','descripcion','modelo','obs');
	...
	$rscount = mysql_query("SELECT count(*) AS cnt FROM `catrefacciones` $where"); 
	...
	$query_recordset = "SELECT id,codigo,descripcion,modelo,obs FROM `catrefacciones` $where $order";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `catrefacciones` (codigo,descripcion,modelo,obs) VALUES (%s,%s,%s,%s)" ,			GetSQLValueString($_REQUEST["codigo"], "text"), # 
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `catrefacciones` WHERE id = %s",
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `catrefacciones` SET codigo = %s,descripcion = %s,modelo = %s,obs = %s WHERE id = %s", 
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `catrefacciones` WHERE id = %s", 
			GetSQLValueString($row_recordset["id"], "int")
		);
