<?php
require_once(dirname(__FILE__) . "/Catdiagnosticosconn.php");
...
function findAll() {
	 ...
	 */
	$fields = array('id','clave','descripcion','modelo','obs');
	...
	$rscount = mysql_query("SELECT count(*) AS cnt FROM `coddiagnostico` $where"); 
	...
	$query_recordset = "SELECT id,clave,descripcion,modelo,obs FROM `coddiagnostico` $where $order";
...
function insert() {
	...
	$query_insert = sprintf("INSERT INTO `coddiagnostico` (clave,descripcion,modelo,obs) VALUES (%s,%s,%s,%s)" ,			GetSQLValueString($_REQUEST["clave"], "text"), # 
			GetSQLValueString($_REQUEST["descripcion"], "text"), # 
			GetSQLValueString($_REQUEST["modelo"], "text"), # 
			GetSQLValueString($_REQUEST["obs"], "text")# 
	);
...
function update() {
	...
	$query_recordset = sprintf("SELECT * FROM `coddiagnostico` WHERE id = %s",
	...
	if ($num_rows > 0) {
		...
		$query_update = sprintf("UPDATE `coddiagnostico` SET clave = %s,descripcion = %s,modelo = %s,obs = %s WHERE id = %s", 
			GetSQLValueString($_REQUEST["clave"], "text"), 
			GetSQLValueString($_REQUEST["descripcion"], "text"), 
			GetSQLValueString($_REQUEST["modelo"], "text"), 
			GetSQLValueString($_REQUEST["obs"], "text"), 
...
function delete() {
	...
	if ($num_rows > 0) {
		$row_recordset = mysql_fetch_assoc($recordset);
		$query_delete = sprintf("DELETE FROM `coddiagnostico` WHERE id = %s", 
			GetSQLValueString($row_recordset["id"], "int")
		);
